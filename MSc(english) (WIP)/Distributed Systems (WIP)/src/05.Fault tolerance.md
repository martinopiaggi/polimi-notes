...
The goal is to enable members of a group to communicate with each other in a reliable manner, ensuring that messages are delivered accurately and in order, even in the presence of faults.
...
The complexity increases when processes can fail or when there are changes in group membership during communication. The **atomic multicast** problem arises: a message is delivered to all non-faulty members of a group or to none, maintaining the same order of messages at all receivers. 
...
Virtual synchrony model distinguishes between receiving a message and delivering it; messages are buffered upon receipt and only delivered when certain conditions are met.
...
It's fundamental that changes to the group view are delivered in a consistent order with respect to other messages. When a view change occurs, all messages must be delivered either before or after this change to maintain consistency.

#### Message ordering

To retain the **virtual synchrony property**, different orderings for multicast messages can be identified:
...
- **Checkpoint-dependency graph**: 
	- the dependency is added from the starting state of the interval to the final state of the other interval (where the arrow arrives).
	- You select the set with the checkpoints nearest the crash: this is called "hypothesis set"
	- if there are no dependencies in the hypothesis a recovery line is found, otherwise the arrival checkpoint of the dependency which is in the hyp set is deleted and a new iteration of the algorithm is done (selecting the set without the checkpoint which is deleted)
	- you continue until you find a set which has a checkpoint for each process and not contain dependencies between these chosen checkpoints.
