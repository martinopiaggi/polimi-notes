...
We can also define synchronization of order $n:m$ as when the phase of oscillator 1 multiplied by $n$ minus the phase of oscillator 2 multiplied by $m$ is a constant value. This condition can also be represented as $n\omega_1 = m\omega_2$.
...
First problem is to just rapresent the curve with a function. 
Three methods:
...
A second method: when the attractor has a suitable geometric structure we can define a vector centered in $0$, formally called "the projector of the attractor". Often is not possible to define a vector (module and direction) $\Phi(t)$ which monotonically increase .

A third method: to find the parametrization which defines me a phase is to define the **crossings of a suitably Poincaré section**: $\Phi(t)$ is defined to (linearly) increase of 2$\pi$ between two consecutive crossings of a suitably defined Poincaré section $\Pi.$ Basically we define a section and every time the oscillator passes through that section we increase by $2\pi$ the phases.
