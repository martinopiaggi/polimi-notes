...
And the **moments of Degree Distribution**: $\langle k^r$ are:
...
In a binary (unweighted) network, the random walker at node $i$ chooses an out-link $i \rightarrow j$ with uniform probability:
...
In directed networks, we can take into account the different role of in-out links.
...
Duality: China as an old strong hub and now authority in the World Trade Network. 
