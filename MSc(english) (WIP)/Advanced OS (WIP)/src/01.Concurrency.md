...
- **Total Store Order (TSO)**: Used in x86 intel processors, involves FIFO store buffers allowing delayed writes. 
	- When these processors issue a store, they utilize a local write queue - also known as a store buffer - to hide memory latency. $L$ is a global lock. use local write queue to hide memory latency, at the moment that a write reaches shared memory, any future read on any processor will see it and use that value, loads appears reordered with respect to stores, processors allow the use of specific "memory barriers" instructions, each thread flushes its previous write to memory before starting its read -> this force sequentially consistent behavior at critical moments in a program. It includes mechanisms like `mfence` and `.L` instructions for managing write buffers.
	- Hardware synchronisation techniques » Fences Memory barriers. In general, to avoid data races you want to reason more intuitively about the memory model behavior of TSO and PSO by enforcing manually SC guarantees. You can do it by using synchronization instructions such as fences or acquire/release sync. Obviously fences are expensive, since you are "blocking the processor". The solution is "balance" and a trade off. Program order for T1 and T2 Memory order. Fences (barriers) are synchronization instructions that strongly enforces program order $<_p$ into memory order $<_m$. A fence (memory barrier) enforces (for adjacent instructions) the memory order to be equal to program order. This solves problems of TSO.
