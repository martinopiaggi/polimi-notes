...
The kernel treats physical pages as the basic unit of memory management. Most 32-bit architectures have 4KB pages, whereas most 64-bit architectures have 8KB pages. The kernel represents every physical page with a struct page structure
...
vmalloc() works in a similar fashion to kmalloc(), except it allocates memory that is only virtually contiguous and not necessarily physically contiguous. The kmalloc() function guarantees that the pages are physically contiguous (and virtually contiguous).

For the most part, only hardware devices require physically contiguous memory allocation. Despite the fact that physically contiguous memory is required in only certain cases, most kernel code uses kmalloc() and not vmalloc(); primarily this is for performance.
...
Linux kernel provides the slab layer (also called the slab allocator), the slab layer acts as a generic data structure caching layer.

For example, one cache is for process descriptors, another cache is for inode objects
...
- A node typically represents a set of CPUs and memory.
- Each node is further divided into zones (e.g., DMA, Normal, HighMem in Linux).
- A zone is a contiguous area of memory within a node.


These caches are not part of the NUMA architecture per se. NUMA primarily concerns the main system memory (RAM) and how it is accessed by different processors.
...
The kernel divides pages into different zones, the kernel uses zones to group pages of similar properties.
