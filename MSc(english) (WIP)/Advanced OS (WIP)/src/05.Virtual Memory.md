...
3. **Slab Allocator**: manages memory allocations for objects of similar sizes to minimize fragmentation. It is used extensively within the kernel where various buffers are used often. 
...
4. The slab allocator provides two main classes of caches:
...
User Space Page Caching is a way to improve system performance in Linux by storing frequently accessed data in memory. 
How works?

- The Page Cache caches data from disk, including file system data and executable code.
- When a file is read, its contents are stored in the cache, allowing subsequent reads to be served from memory instead of reading from the disk. This reduces disk I/O and improves overall performance.
- The page cache is a collection of `struct page`. 
- Each `struct page` is a physical page descriptors Represents the properties of a page in memory contains crucial information like mapping counters, address space references, and status flags.

Two fundamentals **mapping mechanisms**:
...
- **backward mapping**: `struct page -> [VMA]` is useful when we want to invalidate page tables entries of shared pages in different processes which is particularly useful for file-backed pages and Copy-on-Write (CoW) pages. Fields in the `struct page` struct like `_mapcount`, `address_space`, `flags`, and reference counters play a crucial role.
...
The **Page Frame Reclaim Algorithm** complements this categorizing pages into lists based on usage frequency.
Actually it uses two lists of pages in each memory zone: 
- active
- inactive: pages start off in the inactive list and move to the active list after being accessed twice, indicating frequent use. The algorithm tries to remove pages not marked as dirty from the inactive list.
