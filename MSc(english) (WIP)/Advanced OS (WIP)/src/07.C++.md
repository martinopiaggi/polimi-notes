...
1. **Constructor**: initializes an obct anjed can take parameters. If not defined, the compiler will generate a default one.
...
![](images/dcc741d348d84eca5365b23c00904182.png)
...
void flexibleFunction() {
    std::unique_lock<std::mutex> lock(myMutex); // You've locked the door
    // Do some work...
    lock.unlock(); // You can manually unlock the door if needed
    // Do some work without the lock...
    lock.lock(); // And then re-lock it when necessary
}
