...
[07.Virtualization](../../Computing%20Infrastructures/src/07.Virtualization.md) 
...
This theorem states that for any conventional computer, a Virtual Machine Monitor (VMM) can be built if the set of sensitive instructions for that computer is a subset of the set of privileged instructions.

1. Implications of the theorem:
    - The theorem provides a sufficient condition for building a VMM.
    - A VMM can control the execution of guest operating systems and virtual machines by intercepting and emulating privileged instructions. a set of instructions is virtualizable if all sensitive instructions are also privileged. This means that any attempt by the guest OS to execute a sensitive instruction will cause a trap to the VMM, allowing it to handle the instruction appropriately. 
...
x86 processors(pre-VT-x) do not comply with Popek and Goldberg theorem. However, 
...
Yes, KVM (Kernel-based Virtual Machine) can utilize IOMMU (Input-Output Memory Management Unit) for virtualized environments. The IOMMU plays a critical role in enhancing the security and efficiency of device I/O in virtualized systems
...
On the other hand, the IOMMU extends this concept of address translation and memory protection to I/O devices, particularly for Direct Memory Access (DMA) operations. Without an IOMMU, devices performing DMA operations must be given physical memory addresses to read from or write to. This can pose security and stability risks, as a misbehaving or malicious device could access or corrupt memory regions outside its intended bounds.

The IOMMU addresses these challenges by providing a mechanism to apply virtual-to-physical address translation and access control for device DMA, similar to how the MMU does for CPU access to memory. This allows devices to use "virtual addresses" in their operations, which the IOMMU then translates to physical addresses according to translation tables it maintains.
...
DEPRIVILEGING: Ring deprivileging in virtualization refers to the process of reducing the privilege level of a virtual machine by moving it from a higher privilege ring to a lower one, such as from Ring 0 to Ring 3 and 1.

Together with shadowing it is a basic constituent of software virtualisation


giusta: da ring 0 -> ring 3


SHADOWING: in the context of virtual mamemory it's the process of translating physical addresses of the guest into physical ones of the host. 


PARAVIRTUALIZATION: 

Paravirtualization is particularly beneficial in scenarios where the guest operating system can be modified to work closely with the hypervisor, leading to improved performance and reduced virtualization overhead.
