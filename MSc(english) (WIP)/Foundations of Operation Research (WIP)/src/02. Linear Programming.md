...
A function with the decisional variables. We will try to optimize it.
...
- $H={x \in \mathbb{R}^n: a^T x=b}$ is a hyperplane;
- $H^{-}={x \in \mathbb{R}^n: a^T x \leq b}$ is an affine half-space (half-plane in $\mathbb{R}^2$).
...
Consider a $LP \min {c^T x: x \in P}$, where $P \subseteq \mathbb{R}^n$ is a non-empty polyhedron of the feasible solutions (in standard or canonical form). Then either there exists (at least) one optimal vertex or the value of the objective function is unbounded below on $P$.
...
For any polyhedron $P=x \in \mathbb{R}^n: A x=b, x \geq 0$:
...
If $X=\{x \mid A x \geq b, x \geq 0\} \neq \emptyset$ and $\min c^Tx \mid x \in X$ is finite, there exists $x^* \in X$, $y^* \in Y$ such that $c^T x^*=b^T y^*$.
...
$x \in \mathbb{R}^n$ is a basic feasible solution iff $x$ is a vertex of $P={x \in \mathbb{R}^n: A x=b, x \geq 0}$
...
The simplex method with Bland's rule terminates after $\leq\binom{n}{m}$ iterations where $n$ is the number of decision variables and $m$ is the number of constraints.
...
2. while the coefficients are negative you continue to perform the pivoting operation switching the basic variables in the first column with non basic variables:
   ...
   4. the value $k$ differs for each row and it's choose to make all the coefficients in the pivot column $j$ equal to zero.
...
$$
\begin{aligned}
y_i (a_i^T x - b_i) = 0 \quad \text{for } i = 1,\ldots,m \\
(c_j - y^T A_j) x_j = 0 \quad \text{for } j = 1,\ldots,n
\end{aligned}
$$
...
$$
$$
B^{-1}(\underline{b}+\delta_k \underline{e}_k) \geq \underline{0} \Rightarrow B^{-1} \underline{b} \geq-\delta_k B^{-1} \underline{e}_k
$$
