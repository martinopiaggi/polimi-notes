...
& z_{t-1}+x_t+x_t^{\prime}-d_t=z_t \quad t \in T \quad \text{ (inventory balance) } \\
...
Also remember to explicit the constraint: $\sum_j^{\text{number of links}} x_{ij} = 1$ which in case of 2 links was kept 'implicit'.
...
x_1=\text{initial number of workers} & \\
