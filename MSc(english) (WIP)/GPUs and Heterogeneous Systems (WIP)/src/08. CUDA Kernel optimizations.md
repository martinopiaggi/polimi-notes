...
The goal is to hide memory latencies by fully utilizing all available resources, including context, words, and threads. This allows the GPU scheduler to continue executing work, effectively hiding the latency of memory operations from both compute and memory perspectives.
...
This may result in non-coalesced memory access if `blockDim.x` is not a multiple of the memory coalescing granularity (e.g., 32 for current CUDA architectures).

Threads in a warp may access non-contiguous memory locations, leading to multiple memory transactions.

Memory banks in GPU shared memory are organized such that successive 32-bit words are assigned to successive banks. Coalesced access helps avoid bank conflicts, where multiple threads try to access the same bank simultaneously.
...
This version assumes a 2D grid of threads where `BLOCKSIZE` is tuned to the architecture's coalescing requirements. Each thread calculates unique `x` and `y` coordinates corresponding to matrix elements in `C`. The division and modulo operations ensure that threads within the same warp access contiguous memory locations, ensuring memory coalescence.

### Bank Conflicts `

Bank conflicts occur when multiple threads in the same warp attempt to access data from the same memory bank of shared memory simultaneously. 

Bank conflicts occur when multiple threads in a warp access different words in the same memory bank of shared memory.
...
Achieves coalesced reads and writes using shared memory:
...
But this can give bank conflict:
...
This strategy involves rearranging thread-to-work/data mapping and data layout to minimize situations where threads in the same warp take different execution paths. It improves SIMD efficiency by reducing idle cores during SIMD execution.

By using a stride-based approach, threads in the same warp are more likely to have similar workloads, reducing divergence.

Imagine we have a 1D array of data, and we want to compute a histogram. We'll represent it like this:
...
Now, let's say we have 4 threads in a warp (in reality, it's 32, but 4 is easier to visualize). 

Without the stride-based approach, the threads might work like this:
...
This can lead to divergence if, for example, only some of these elements fall within the valid alphabet range.

With the stride-based approach:
...
3. If there's any pattern in the data (e.g., certain parts of the array are more likely to be in the valid alphabet range), all threads will encounter this pattern together.

This approach reduces divergence because:

- Threads in the same warp are more likely to take the same path through the if-statement (alphabet_position >= 0 && alphabet_position < ALPHABET_SIZE) at the same time.
...
**Tiling** involves dividing a large data set into smaller chunks, or tiles, and processing them in parallel. This can help to hide memory latencies and improve occupancy.


Tiling is useful for matrix multiplication but can also be used for other parallel patterns across all the data.  
...
Thread coarsening involves assigning multiple units of parallelism to each thread. This reduces redundant work, divergence, or synchronization, and decreases redundant global memory traffic.
