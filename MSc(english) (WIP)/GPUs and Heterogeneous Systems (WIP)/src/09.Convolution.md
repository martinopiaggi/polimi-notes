...
- **Constant Memory Benefits:**
	- A constant memory cannot be modified by threads during kernel execution ○ The size of the constant memory is quite small (64KB) and efficient ■ It is a read only cache ● Less complex hardware to manage it ● good performance only when each thread accesses the same data
...
Caching Halo cells ○ Similar to the previous approach about tiling ○ Halo cells are data that come from other tiles ○ Upon running it is very likely that these data are already available in the L2 cache
