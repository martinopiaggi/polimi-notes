...
`cudaMalloc()` and `cudaMemcpy()` are generally simpler to use and understand, especially for beginners. Pinned memory requires more careful management but can offer performance benefits in certain scenarios. `cudaMalloc()`/`cudaMemcpy()` may involve an extra copy step, as the GPU driver **might** first copy from pageable host memory to a **pinned buffer**, then to device memory. Pinned memory eliminates this intermediate copy, potentially improving transfer speeds.
...
- Blocking operation (waits for copy to complete)
- Handles different types of memory transfers
- Ensure source and destination have enough allocated space
...
//Block the CPU thread until all previously issued CUDA calls have completed on the GPU.
//The CHECK macro wraps this call to check for any errors
//in the other instructions here, the CHECK() is omitted
...
4. Remember:
   ...
   - Match cudaMalloc() with cudaFree() to avoid memory leaks
   - Ensure correct cudaMemcpyKind for the transfer direction
   - Size in cudaMemcpy() should not exceed allocated memory
