...
2. **Convolutional Layer (Conv)**: 
   - Parameters (|par|) = |F| * (k_w * k_h * d_in + 1)
   - Where |F| is the number of filters, k_w and k_h are the kernel width and height, and d_in is the depth of the input layer.
   - The "+1" accounts for the bias term for each filter.

3. **Dense Layer**: 
   - Parameters (|par|) = d_out * (d_in + 1)
   - Where d_out is the number of neurons in the dense layer, and d_in is the number of inputs to the neuron.
   - The "+1" accounts for the bias term for each neuron.

4. **Batch Normalization**: 
   - Parameters (|par|) = 4 * d_in
   - Where d_in is the depth of the input.
   - Accounts for gamma, beta, moving mean, and moving variance.

5. **Pooling Layers** (MaxPooling, AveragePooling): 
   - Parameters (|par|) = 0
   - These layers do not have learnable parameters.

#### General Rules for Output Shapes:

1. **Convolutional Layer**:
   - Output shape = (w_out, h_out, d_out)
   - Where w_out = ceil((w_in + (k_w - 1) * (p - 1)) / s_w)
   - h_out = ceil((h_in + (k_h - 1) * (p - 1)) / s_h)
   - d_out is the number of filters in the Conv layer.
   - p is padding, s_w, and s_h are strides in width and height.

2. **Pooling Layers**:
   - Output shape = (w_out, h_out, d_out)
   - Usually, k = s = 2 for default pooling layers.
   - d_out is the same as d_in (depth doesn't change).

3. **Flatten Layer**:
   - Output shape = (w_in * h_in * d_in)
   - Flattens the input to a single vector.

4. **Dense Layer**:
   - Output shape = (d_out)
   - Dependent on the number of neurons in the layer.
